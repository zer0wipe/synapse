...
export class BranchSidebarView extends ItemView {
  ...
  async onOpen() {
    ...
    this.container = contentEl.createDiv({
      cls: "synapse-branch-sidebar__content",
    });
    this.renderBranch();
  ...
  private renderEmptyState() {
    const emptyState = this.container.createDiv({
      cls: "synapse-branch-sidebar__empty",
    });
    emptyState.setText(
      "Branch is empty. Add notes via links or command palette.",
    );
  }

  private renderList(entries: BranchEntry[]) {
    const list = this.container.createEl("ul", {
      cls: "synapse-branch-sidebar__list",
    });
    const total = entries.length;
    entries.forEach((entry, index) => {
      list.appendChild(this.createListItem(entry, index, total));
    });
  }
  ...
  private createListItem(
    ...
    const removeButton = li.createEl("button", {
      cls: "synapse-branch-sidebar__remove",
    });
